...
async def blacklist_chat_func(client, message: Message):
    try:
        if len(message.command) != 2:
            return await message.reply_text(font(" **Usage:** `/blchat <chat_id>`"))
        
        try:
            chat_id = int(message.text.strip().split()[1])
        except ValueError:
            return await message.reply_text(font(" **Invalid chat ID!**"))
        ...
        try:
            ...
            await message.reply_text(f" **Bot is not in chat `{chat_id}`**")
        except Exception:
            await message.reply_text(f" **Couldn't leave chat `{chat_id}` - bot not in chat**")
...
async def whitelist_chat_func(client, message: Message):
    try:
        if len(message.command) != 2:
            return await message.reply_text(font(" **Usage:** `/unblchat <chat_id>`"))
        ...
        if whitelisted:
            return await message.reply_text(f" **Chat `{chat_id}` has been removed from blacklist!**")
